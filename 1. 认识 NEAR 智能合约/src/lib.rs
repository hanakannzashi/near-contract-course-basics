...
#[derive(BorshDeserialize, BorshSerialize, PanicOnDefault)] // 实现 borsh 序列化，实现不可用的 `default` 方法
pub struct Contract {
    owner_id: AccountId,                      // 合约所有者
    descriptions: HashMap<AccountId, String>, // 账户及其描述信息。注：`HashMap` 作为容器不是最好的选择，此处仅用于教学目的
}
